...
// Don't duplicate me!
if ( !class_exists ( 'ReduxFramework_slides_v2' ) ) {
     ...
     * @since       1.0.0
     */
    class ReduxFramework_slides_v2 {
        ...
        public function render () {
            ...
            $defaults = array(
                ...
                'content_title' => esc_html__( 'Slide', 'wp-coupon' )
            );

            $this->field = wp_parse_args ( $this->field, $defaults );

            echo '<div class="redux-slides-accordion" data-new-content-title="' . esc_attr ( sprintf ( esc_html__ ( 'New %s', 'wp-coupon' ), $this->field[ 'content_title' ] ) ) . '">';
            ...
            if ( isset ( $this->value ) && is_array ( $this->value ) && !empty ( $this->value ) ) {
                ...
                foreach ( $slides as $slide ) {
                    ...
                    if( $this->field[ 'show' ][ 'image' ] ) {
                        ...
                        echo '<span class="button media_upload_button" id="add_' . $x . '">' . esc_html__('Upload', 'wp-coupon') . '</span>';
                        ...
                        echo '<span class="button remove-image' . $hide . '" id="reset_' . $x . '" rel="' . $slide['attachment_id'] . '">' . esc_html__('Remove', 'wp-coupon') . '</span>';
                    ...
                    $placeholder = ( isset ( $this->field[ 'placeholder' ][ 'title' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'title' ] ) : esc_html__ ( 'Title', 'wp-coupon' );
                    ...
                    if ( $this->field[ 'show' ][ 'description' ] ) {
                        $placeholder = ( isset ( $this->field[ 'placeholder' ][ 'description' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'description' ] ) : esc_html__ ( 'Description', 'wp-coupon' );
                    ...
                    $placeholder = ( isset ( $this->field[ 'placeholder' ][ 'url' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'url' ] ) : esc_html__ ( 'URL', 'wp-coupon' );
                    ...
                    echo '<li><a href="javascript:void(0);" class="button deletion redux-slides-remove">' . esc_html__ ( 'Delete', 'wp-coupon' ) . '</a></li>';
            ...
            if ( $x == 0 ) {
                echo '<div class="redux-slides-accordion-group"><fieldset class="redux-field" data-id="' . $this->field[ 'id' ] . '"><h3><span class="redux-slides-header">' . esc_attr ( sprintf ( esc_html__ ( 'New %s', 'wp-coupon' ), $this->field[ 'content_title' ] ) ) . '</span></h3><div>';
                ...
                if( $this->field[ 'show' ][ 'image' ] ) {
                    ...
                    echo '<span class="button remove-image' . $hide . '" id="reset_' . $x . '" rel="' . $this->parent->args['opt_name'] . '[' . $this->field['id'] . '][attachment_id]">' . esc_html__('Remove', 'wp-coupon') . '</span>';
            ...
            echo '</div><a href="javascript:void(0);" class="button redux-slides-add button-primary" rel-id="' . $this->field[ 'id' ] . '-ul" rel-name="' . $this->field[ 'name' ] . '[title][]' . $this->field['name_suffix'] .'">' . sprintf ( esc_html__ ( 'Add %s', 'wp-coupon' ), $this->field[ 'content_title' ] ) . '</a><br/>';
        }
        ...
        public function enqueue () {
            ...
            $url = get_template_directory_uri().'/inc/redux-extensions/slides_v2/slides_v2/';

            wp_enqueue_style ('redux-field-media-css');

            wp_enqueue_style (
                'redux-field-slides-v2-css',
                $url . 'field_slides_v2.css',
                array(),
                time (),
                'all'
            );
            ...
            wp_enqueue_script (
                'redux-field-slides-v2-js',
                $url . 'field_slides_v2.js',
